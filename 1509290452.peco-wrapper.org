...
#+BLOG: rubikitch
#+CATEGORY: Emacs, Ruby, helm, peco
#+DESCRIPTION: pecoを端末以外から実行できるようにするラッパースクリプト。
#+MYTAGS: relate:helm, relate:anything, relate:shell, relate:eshell, シェルでanythingの絞り込み, シェルでhelmの絞り込み, peco, percol, go, golang, zsh, bash, zaw, ターミナル版anything的なpercol, ターミナル版anything的なpeco, 端末版Unite.vimなpeco, pecoをemacsで動かす, pecoをeshellで動かす, , 端末エミュレータでpecoを動かす, ターミナルエミュレータでpecoを動作させる
#+TITLE: 端末でhelm風絞り込み候補選択をするpecoを端末外から実行できるようにした
#+begin: org2blog-tags
#+TAGS: relate:helm, relate:anything, relate:shell, relate:eshell, シェルでanythingの絞り込み, シェルでhelmの絞り込み, peco, percol, go, golang, zsh, bash, zaw, ターミナル版anything的なpercol, ターミナル版anything的なpeco, 端末版Unite.vimなpeco, pecoをemacsで動かす, pecoをeshellで動かす, , 端末エミュレータでpecoを動かす, ターミナルエミュレータでpecoを動作させる, Emacs, Ruby, helm, peco, --xterm, --xterm
#+end:
