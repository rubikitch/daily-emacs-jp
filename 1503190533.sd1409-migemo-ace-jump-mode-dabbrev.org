...
#+BLOG: rubikitch
#+CATEGORY: Emacs, SKK, 連載『るびきち流Emacs超入門』
...
#+MYTAGS: package:ace-jump-mode, package:ddskk, package:ace-jump-mode, package:hippie-exp, package:dabbrev, package:migemo, hippie-expand, ace-jump-mode, cmigemo, migemo.el
#+TITLE: #5 カーソル移動と入力支援でスピードアップ！ (Software Design 2014年9月号掲載記事) Emacs Migemo ace-jump-mode dabbrev hippie-exp
#+begin: org2blog-tags
#+TAGS: package:ace-jump-mode, package:ddskk, package:ace-jump-mode, package:hippie-exp, package:dabbrev, package:migemo, hippie-expand, ace-jump-mode, cmigemo, migemo.el, Emacs, SKK, 連載『るびきち流Emacs超入門』, helm, 文字入力の一元化, カーソル移動, 効率的な文字入力, 変数名, 関数名, シンボル, 文字列リテラル, 対応する括弧の末尾, S式, インクリメンタルサーチ, isearch, 正規表現isearch, 正規表現, メタ文字, 漢字変換, 漢字変換なしでローマ字で日本語文字列をisearch, Migemo, cmigemo, migemo.el, 入力支援機能, dabbrev, M-/, 補完, SKK, ファイル名補完, hippie-expand-try-functions-list, try-complete-file-name-partially, try-complete-file-name, try-expand-line, try-expand-list, 単語登録, 英字変換
#+end:
...
4. 2014/09 [[http://rubikitch.com/2015/03/16/sd1408-ddskk/][SKK＋AZIKで快適・効率的な日本語入力を！]]
...
本サイトのレビュー記事もあります。
- [[http://rubikitch.com/2014/08/20/migemo/][Migemo(cmigemo+migemo.el)]]
  - 多くのelispで対応しています→ [[http://rubikitch.com/tag/migemo][tag:migemo]]
- [[http://rubikitch.com/2014/10/09/ace-jump-mode/][ace-jump-mode.el]]
  - ace-jump-modeを使った応用例→ [[http://rubikitch.com/tag/ace-jump-mode/][tag:ace-jump-mode]]
- *helm* ユーザならば断然 [[http://rubikitch.com/2014/12/25/helm-swoop/][helm-swoop.el (Emacs Advent Calendar 2014記事)]]
* カーソル移動
...
　[[http://rubikitch.com/2015/03/16/sd1408-ddskk/][先月紹介した日本語入力システムSKK]] は単語変換だからこそ独自のメリットが存在します。特筆すべきはすぐに使える単語登録と英字変換です。SKK辞書は品詞情報がないため、見出し語と変換結果のテーブルに過ぎません。つまり、好き勝手に *単語登録* しまくれるのです。
