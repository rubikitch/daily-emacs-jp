...
#+DESCRIPTION:
#+TAGS:mark, yank, kill-ring, mark-ring, exchange-point-and-mark, set-mark-command-repeat-pop, global-mark, mark-word, mark-sexp, mark-whole-buffer, rectangle-mark-mode, キーボードマクロ, kmacro, centimacro, relate:centimacro, capitalize, relate:sequential-command, package:sequential-command, count-words-region, count-lines-page, what-cursor-position, find-library, recenter-top-bottom, balance-windows, shrink-window-if-larger-than-buffer, text-scale-adjust, set-selective-display, package:cn-outline, 
