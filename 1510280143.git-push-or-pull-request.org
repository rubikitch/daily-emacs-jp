...
#+MYTAGS: git, github, hub pull-request, git push, vc, relate:vc, emacs vc git, C-x v p, relate:magit, emacs git pull-request, make-comint, shell-mode, comint-mode, comint-run
#+TITLE: 自動判別でGitHubにpush/pull-requestするスクリプトを作りました(Emacsの設定付)
#+begin: org2blog-tags
#+TAGS: git, github, hub pull-request, git push, vc, relate:vc, emacs vc git, C-x v p, relate:magit, emacs git pull-request, make-comint, shell-mode, comint-mode, comint-run, Emacs, 自作プログラム, 
#+end:
